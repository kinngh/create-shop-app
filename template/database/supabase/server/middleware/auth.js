...
const { upsertStore } = require("../../utils/models/StoreModel.js");
const { deleteSession } = require("../../utils/models/SessionModel.js");
...
const authMiddleware = (app) => {
  ...
  app.get("/auth/callback", async (req, res) => {
    try {
      ...
      await upsertStore({ shop, isActive: true });

      res.redirect(`/?shop=${shop}&host=${host}`);
    } catch (e) {
      const { shop } = req.query;
      //MARK:- MAKE SHOP INACTIVE AND DELETE ANY STORED SESSION DATA
      await upsertStore({ shop, isActive: false });
